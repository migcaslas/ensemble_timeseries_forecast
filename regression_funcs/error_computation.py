...
class ErrorComputation(object):
    ...
    def RMSE_instances(cls, forecast_y_matrix, y_test_matrix):
        return numpy.sqrt(numpy.nanmean(cls._percentage_error(forecast_y_matrix, y_test_matrix)**2, axis=1))

    @classmethod
    def RMSE(cls, forecast_y, y_test):
        return numpy.sqrt(numpy.nanmean(cls._percentage_error(forecast_y, y_test)**2))
